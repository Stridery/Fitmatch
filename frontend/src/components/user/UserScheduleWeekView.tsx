...
interface UserScheduleWeekViewProps {
  weekStart: Date;
  events: EventData[];
}

export default function UserScheduleWeekView({ weekStart, events }: UserScheduleWeekViewProps) {
            ...
            <div key={dayIndex} className="bg-white dark:bg-gray-800 relative">
              ...
              {events.map((event) => {
                ...
                return (
                  <div
                    key={event.id}
                    className={`absolute left-1 right-1 ${getEventColor(event.kind)} text-white text-xs p-1 rounded shadow-sm`}
                    style={{
